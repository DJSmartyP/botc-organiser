...
The dedicated **Chaos on the Clocktower** watch page presents the complete 16-episode collection in Episode 1–16 order using locally hosted, user-supplied JPEG artwork, so the library does not depend on YouTube's thumbnail service. Before an episode is selected, a local `assets/chaos-theatre.png` scene and the real Chaos logo form a responsive **Chaos Theatre** holding poster. Selecting a card opens an in-theme episode dossier with the full artwork, title and a **Scripts this episode** panel; the privacy-enhanced `youtube-nocookie.com` player is created only after the viewer presses **Play episode**. Each episode accepts any number of `{ name, startSeconds, notes, resourceUrl, resourceLabel }` script entries. Supplied start times become accessible buttons that launch the video at the matching moment. A verified `resourceUrl` adds a separate view/download link; omit it when no exact public source has been confirmed so the interface never offers a misleading or broken result. Playback includes playlist navigation, fullscreen support, an active selection indicator and a direct playlist link. The collection is intentionally curated in `CHAOS_EPISODES` in `app.js`; update that list and `assets/episodes/` only if the published collection changes.
