...
The dedicated **Chaos on the Clocktower** watch page presents the complete 16-episode collection in Episode 1–16 order using locally hosted, user-supplied JPEG artwork, so the library does not depend on YouTube's thumbnail service. Before an episode is selected, a local `assets/chaos-theatre.png` scene and the real Chaos logo form a responsive **Chaos Theatre** holding poster. Selecting a card opens an in-theme episode dossier with the full artwork and title; the privacy-enhanced `youtube-nocookie.com` player is created only after the viewer presses **Play episode**. A persistent **Scripts this episode** panel sits below the player. Each episode accepts any number of `{ name, startSeconds, tags }` script entries. Supplied start times power script-name buttons that launch the video at the matching moment without displaying the time. Add tags explicitly or place trailing notes in parentheses in the script name; both forms render as colour-coded badges. Game 1, Game 2, Game 3, Veiled and Smarty tags have fixed colours, while new tag names receive a stable fallback colour. Playback includes playlist navigation, fullscreen support, an active selection indicator and a direct playlist link. The collection is intentionally curated in `CHAOS_EPISODES` in `app.js`; update that list and `assets/episodes/` only if the published collection changes.
