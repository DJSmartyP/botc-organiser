...
The dedicated **Chaos on the Clocktower** watch page presents the complete 16-episode collection in Episode 1–16 order using locally hosted, user-supplied JPEG artwork, so the library does not depend on YouTube's thumbnail service. Before an episode is selected, a local `assets/chaos-theatre.png` scene and the real Chaos logo form a responsive **Chaos Theatre** holding poster. Selecting a card opens an in-theme episode dossier with the full artwork, title, individual episode description, script count and compact format summary; the privacy-enhanced `youtube-nocookie.com` player is created only after the viewer presses **Play episode**. A persistent **Script chapter marks** panel sits below the player with a clear reminder that selecting a script jumps directly to its starting point. The curated archive currently contains 46 timestamped script chapters across all 16 episodes. Supplied start times power script-name buttons that launch the video at the matching moment without displaying the time. Trailing notes in parentheses are removed from the script title and rendered inside the same button as colour-coded badges. Game-count, Base 3, Teensy, Full Homebrew, Special Game Mode, Veiled Game and Smarty Player Perspective tags have consistent semantic colours; new tag names receive a stable fallback colour. Each embed is deliberately limited to its selected episode so YouTube's internal next/previous controls cannot advance independently and leave the chapter panel out of sync. Playback includes fullscreen support, an active selection indicator and a separate direct playlist link. The collection is intentionally curated in `CHAOS_EPISODES` in `app.js`; update that list and `assets/episodes/` only if the published collection changes.
