...
try {
  ...
  for (const viewport of [{ width: 390, height: 844, name: "mobile" }, { width: 1440, height: 900, name: "desktop" }]) {
    ...
    assert.deepEqual(await watchPage.locator(".episode-script-name").allTextContents(), ["Catfishing", "Kaboom!"]);
    assert.deepEqual(await watchPage.locator(".episode-script-tag").allTextContents(), ["Multiple Games"]);
    assert.equal(await watchPage.locator(".episode-script-tag--game-count").count(), 1);
